...
**Phase 0 + Phase 1 + Phase 2 shipped 2026-05-06 / 2026-05-08** (commits `f3b39b1`, `b86ee94`, and Phase 2 chain; package version `1.0.0.dev2`). Subsequent phase build guides land as their phases ship: Phase 3 (Control + Orchestrate), Phase 4 (Router), Phase 5 (verification gate), Phase 6 (safety gate + identity + state + queue), Phase 7 (audit + ledger + drift), Phase 8 (admin), Phase 9 (adapters + MCP + CLI), Phase 10 (OTel + cloud seams + standalone binary), Phase 11 (release).
...
| Phase 2 (Solver wiring through Trinity Core pipeline) | **Shipped 2026-05-08** (package version `1.0.0.dev2`) -- `WobbleAxis` Protocol + `CrossPrecisionAxis` (Axis 1), engine adapter, Solver-only `solve()` orchestrator, `POST /v1/tasks`. 34 tests passing. |
| Trinity Core (physics heart) | Solver vendored + wired through pipeline (Phase 2); Control vendored at Phase 1, wiring pending Phase 3; Orchestrate (greenfield) pending Phase 3 |
| Front door (REST + WebSocket + CLI + MCP) | `/v1/health` (Phase 0) + `POST /v1/tasks` (Phase 2) live; full surface pending Phase 5+ |
