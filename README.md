...
**All v1 phases shipped 2026-05-06 → 2026-05-14** (Phase 0 → Phase 11; package version `1.0.0.dev11`, releasing to `1.0.0.dev12` at Phase 11 acceptance). Phase 11 closes v1 with the canonical Section 10.7 acceptance battery: four panic-mode isolation + composition tests (NATS-down, state-backend-down, drift-detector-down, simultaneous-three) plus a long-window bit-exact replay test, all marked `@pytest.mark.acceptance`. Per-directory READMEs filled across `phoenix/` and `vendor/`.
