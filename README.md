...
**Phase 0 + 1 + 2 + 3 + 4 + 5 shipped 2026-05-06 / 2026-05-08** (commits `f3b39b1`, `b86ee94`, Phase 2 chain, Phase 3 chain, Phase 4 chain, Phase 5 chain; package version `1.0.0.dev5`). Subsequent phase build guides land as their phases ship: Phase 6 (safety gate + identity + state + queue), Phase 7 (audit + ledger + drift), Phase 8 (admin), Phase 9 (adapters + MCP + CLI), Phase 10 (OTel + cloud seams + standalone binary), Phase 11 (release).
...
| Phase 5 (Verification gate + rung table + Axis 3 + classifier) | **Shipped 2026-05-08** (package version `1.0.0.dev5`) -- `VerificationGate.verify(task)` with adaptive rung selection (R1–R5 per Section 6.4); reactive promotion (max 2 per task); `CrossProviderAxis` (Axis 3) at R4+ with double-routing via Router; `PhoenixDisagreementType` extension (CONVERGED / NUMERICAL_DRIFT / BACKACTION_SENSITIVE / PROVIDER_DIVERGENT / DEGRADED / DEGRADED_BUDGET_BOUND); drift-state stub + fail-closed wiring per Section 6.8; real eigenstate plumbing for QHO Tier-1 (replaces \\|0><0\\| placeholder); real 〈σ_z〉 observable in LocalClassicalSimulator (replaces trace placeholder); `VerificationProvenance` records initial/final rung + promotions + distance matrix + wobble_score sigma. 91 tests passing. |
