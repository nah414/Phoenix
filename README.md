...
**Phase 0 + 1 + 2 + 3 + 4 shipped 2026-05-06 / 2026-05-08** (commits `f3b39b1`, `b86ee94`, Phase 2 chain, Phase 3 chain, Phase 4 chain; package version `1.0.0.dev4`). Subsequent phase build guides land as their phases ship: Phase 5 (verification gate), Phase 6 (safety gate + identity + state + queue), Phase 7 (audit + ledger + drift), Phase 8 (admin), Phase 9 (adapters + MCP + CLI), Phase 10 (OTel + cloud seams + standalone binary), Phase 11 (release).
...
| Phase 4 (Router subsystem + Provider Registry) | **Shipped 2026-05-08** (package version `1.0.0.dev4`) -- seven-stage `Router.decide(routing_request)` algorithm, `ProviderRegistry` with HEALTHY/DEGRADED/OFFLINE health states, 3 quantum provider stubs (IBM/Braket/IonQ; cloud SDK wiring deferred), `LocalClassicalSimulator` registered as default; pricing v1 data + loader; intelligence layer Source A (static `HardwareParams`); equivalence registry (Section 4.5 conservative defaults); failover protocol with exponential-backoff quarantine; pipeline integration replaces Phase 3's default-helper with real Router routing. 75 tests passing. |
| Trinity Core (physics heart) | All three subsystems wired end-to-end (Phase 3); cross-provider verification (Axis 3) deferred to Phase 5 alongside the rung table |
